...
def get_data(serial_input):
    ...
    time_mid = '2022-01-29T11:00' #'%Y-%m-%dT%H:%M'
    ...
    series_opensky = pd.date_range(start=(time_mid).strftime('%Y-%m-%d %H:%M:%S'),
                                    end=(time_mid + timedelta(minutes=duration)).strftime('%Y-%m-%d %H:%M:%S'), freq="30s").strftime('%Y-%m-%d %H:%M:%S')
    ...
    df.to_csv('scrap_30s_{}.csv'.format(serial_input))
